...
static void ui_draw_infobar(UIState *s) {
  ...
  snprintf(pedalpos, sizeof(pedalpos), "%-i %%", (int)(s->b.pedalPos));
  ...
  if (s->scene.lead_status) {
    ...
    snprintf(lead_dist, sizeof(lead_dist), "%-s", "-- m");
  }
  ...
  snprintf(
    infobar,
    sizeof(infobar),
    "%04d/%02d/%02d %02d:%02d:%02d | %s %s | PDL: %s | DST: %s | ANG: %s",
    tm.tm_year + 1900,
    tm.tm_mon + 1,
    tm.tm_mday,
    tm.tm_hour,
  ...
  nvgFontSize(s->vg, hasSidebar? 38:48);
